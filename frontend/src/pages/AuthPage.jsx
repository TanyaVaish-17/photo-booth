...
export default function AuthPage() {
  ...
  return (
    <PageLayout>
      {/* Original: min-h-[80vh] flex items-center justify-center px-4 py-16 — preserved */}
      <div className="min-h-[80vh] flex items-center justify-center px-4 py-10 md:py-16">
        {/* Original card: max-w-md, rounded-3xl, p-8 — preserved on desktop, slightly adjusted on mobile */}
        <div className="bg-white/90 backdrop-blur rounded-2xl md:rounded-3xl shadow-2xl p-6 md:p-8 w-full max-w-md border border-pink-100">

          {/* Header */}
          <div className="text-center mb-6 md:mb-8">
            <span className="text-5xl">🎀</span>
            <h2 className="text-2xl md:text-3xl font-extrabold text-pink-700 mt-3 mb-1">
              {mode === "login" ? "Welcome Back!" : "Join K-Click Booth"}
            </h2>
            <p className="text-pink-400 text-sm">
              {mode === "login" ? "Sign in to see your memories 💕" : "Create an account to save your K-moments 💖"}
            </p>
          </div>
            ...
            {mode === "signup" && (
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                  placeholder="Your cute name 🌸" required
          ...
          <p className="text-center text-sm text-pink-400 mt-6">
            {mode === "login" ? (
              <>Don't have an account?{" "}
                <button onClick={() => { setMode("signup"); setError(""); }} className="text-pink-600 font-bold hover:underline">Sign Up</button>
              </>
            ) : (
              <>Already have an account?{" "}
                <button onClick={() => { setMode("login"); setError(""); }} className="text-pink-600 font-bold hover:underline">Sign In</button>
              </>
            )}
          </p>
        </div>
